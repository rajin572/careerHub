import React from 'react'

const AppliedJobs = () => {
    return (
        <div className='w-[95%] lg:w-[85%] mx-auto'>
            <div className='my-10'>
                <select className='inline-block' id="data">
                    <option value="Filter By">Filter By</option>
                    <option value=" remote option"> remote option</option>
                    <option value="on-site option">on-site option</option>
                </select>
            </div>
            <div className='flex justify-between items-center p-3 gap-5 border rounded my-5'>
                <img className='bg-[#F4F4F4] p-5' src="data:image/png;base64," alt="" />
                <div className='flex-grow'>
                    <h1 className='font-bold'>Frontend Engineer</h1>
                    <p className='text-gray-700'>Google</p>
                    <div className='flex justify-start gap-5 items-center'>
                        <p>Dhaka, Bangladesh</p>
                        <p>Salary: </p>
                    </div>
                </div>
                <button className='px-6 py-3 bg-[#7E90FE] rounded text-white'>View Details</button>
            </div>
        </div>
    )
}

export default AppliedJobs